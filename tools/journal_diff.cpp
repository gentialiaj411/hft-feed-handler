...
int main(int argc, char** argv) {
  ...
  if (!open_map(lhs, lfd, ln, lb) || !open_map(rhs, rfd, rn, rb)) {
    std::printf("failed to open/map journal(s)\n");
    return 1;
  }

  constexpr std::size_t kHdr = sizeof(mf::journal::JournalHeader);
  constexpr std::size_t kRec = sizeof(mf::journal::JournalRecord) + sizeof(mf::core::BookEvent);
  if (ln < kHdr || rn < kHdr) {
    std::printf("invalid journal header size\n");
    return 1;
  }
  if (std::memcmp(lb, rb, kHdr) != 0) {
    std::printf("header mismatch\n");
    return 1;
  }
  const std::size_t lrecs = (ln - kHdr) / kRec;
  const std::size_t rrecs = (rn - kHdr) / kRec;
  if (lrecs != rrecs) {
    std::printf("record_count_mismatch lhs=%zu rhs=%zu\n", lrecs, rrecs);
    return 1;
  }
  for (std::size_t i = 0; i < lrecs; ++i) {
    ...
    if (std::memcmp(lrec, rrec, kRec) != 0) {
      std::printf("diverged_at_record=%zu\n", i);
      return 1;
    }
  }
  std::printf("identical records=%zu\n", lrecs);
